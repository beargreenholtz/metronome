...
import MetronomePick from './MetroPick';
...
export default class Metronome extends React.Component {
  ...
  scheduleNote(note) {
    ...
    gainNode.gain.setTargetAtTime(
      0,
      this.state.nextClickTime + this.state.noteLength,
      0.015
    );
  ...
  handleTick() {
    ...
    while (
      this.state.nextClickTime <
      this.audioContext.currentTime + this.state.scheduleAheadTime
    ) {
      ...
      this.setState((state) => {
        ...
        return {
          ...
          nextClickTime:
            state.nextClickTime +
            (1 / this.state.clicksPerBeat) * secondsPerBeat,
  ...
  handleClick(e) {
    ...
    switch (e.target.name) {
      case 'increment-tempo':
        ...
        this.setState((state) => ({
          tempo: nextTempo > state.maxTempo ? state.tempo : nextTempo,
        }));
        return;
      case 'decrement-tempo':
        nextTempo = parseInt(this.state.tempo) - 1;
        this.setState((state) => ({
          tempo: nextTempo < state.minTempo ? state.tempo : nextTempo,
        }));
        return;
      default:
        break;
